...
<%String id= request.getAttribute("pageBean.currentPage").toString(); 
%>
   <s:if test="%{pageBean.currentPage != 1}"><jsp:include page="highSearch2.jsp"></jsp:include></s:if>
    <form id="form2">
  <table style="width:900px" border="1">
      ...
      <th>操作</th>
         ...
         <% int x=0;%>
       <s:iterator var="ifn" value="#request.list"  status="st">             		
     			<tr>
     			<td><% x++;%><%=x%></td>
   ...
   <center>
     <s:if test="%{pageBean.currentPage == 1}">首页 上一页</s:if>
	       <s:else>
		        <a href="highSearch.action?page=1">首页</a>
		        <a href="highSearch.action?page=<s:property value="%{pageBean.currentPage-1}"/>" >上一页</a>
                   </s:else>
	
	      <s:if test="%{pageBean.currentPage != pageBean.totalPage}">
		<a href="highSearch.action?page=<s:property value="%{pageBean.currentPage+1}"/>">下一页</a>
		<a href="highSearch.action?page=<s:property value="pageBean.totalPage"/>">尾页</a>
	      </s:if>
	      <s:else> 
		下一页	 尾页 
　　　　  </s:else>
